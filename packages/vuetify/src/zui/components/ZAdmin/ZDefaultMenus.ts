...
import {
  ZMenu, ZIcon, ZCard,
  ZList, ZListGroup, ZListItem, ZListItemContent, ZListItemIcon, ZListItemTitle, ZListItemGroup,
} from '../../../components'

import './styles/ZDefaultMenus.scss'
...
export const ZDefaultMenus = Vue.extend({
  ...
  methods: {
    ...
    genFloatMenus (h: CreateElement, type: 'href' | 'path' | 'group' | 'static', item: ZMenuOption): VNode {
        ...
        case 'group': {
          ...
          if (item.children) {
            item.children.forEach(i => {
              listItems.push(h(ZListItem,
                {},
                [
                  h(ZListItemIcon, { staticClass: 'mr-4' }, [h(ZIcon, { staticClass: '', props: { size: 16 } }, [i.icon])]),
                  h(ZListItemContent, [h(ZListItemTitle, { staticClass: '' }, [i.name])]),
                ]
              ))
            })
          }
          content = h(ZList, {
            staticClass: '',
            props: { dense: true, subheader: true, minWidth: 160 },
          }, [
     ...
     * 生成带链接的菜单
     */
    genHrefMenu (h: CreateElement, item: ZMenuOption, children: VNode[]): VNode[] {
      return [
        h(ZListItem, {
          attrs: {
            menuKey: item.key,
     ...
     * 生成带路由的菜单
     */
    genPathMenu (h: CreateElement, item: ZMenuOption, children: VNode[]): VNode[] {
      return [
        h(ZListItem, {
          attrs: {
            menuKey: item.key,
        ...
        // this.genFloatMenus(h, 'path', item),
      ]
    },
    /**
     * 生成静态菜单
     */
    genStaticMenu (h: CreateElement, item: ZMenuOption, children: VNode[]): VNode[] {
      return [
        h(ZListItem, {
          attrs: {
            menuKey: item.key,
     ...
     * 生成菜单组
     */
    genGroupMenu (h: CreateElement, item: ZMenuOption, children: VNode[]): VNode[] {
      const activatorSlot = h('template', { slot: 'activator' }, children)
      const mainNavMiniMode = this.$themeStore.mainNavMiniMode
      const childrenNodeList: VNode[] = []
      ...
      return [
        ...
        // this.genFloatMenus(h, 'group', item),
      ]
    },
    /**
     * 生成菜单
     */
    genMenu (h: CreateElement, item: ZMenuOption): VNode[] {
      /* 如果是隐藏的 */
      if (item.visible) {
        return []
      }
      ...
      const titleNodes = [
        h(ZListItemIcon, titleOptions, [h(ZIcon, { staticClass: activeClass, props: { size: iconSize } }, [iconName])]),
        h(ZListItemContent, [h(ZListItemTitle, { staticClass: activeClass }, [item.name])]),
      ]
  ...
  render (h): VNode {
    const list: VNode[] = []
    const staticClass: string[] = ['z-default-main-menu']
    this.menuData.forEach((item: ZMenuOption) => {
