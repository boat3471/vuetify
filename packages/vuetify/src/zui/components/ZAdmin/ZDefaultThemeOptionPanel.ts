...
import './styles/ZDefaultThemeOptionPanel.scss'
...
export const ZDefaultThemeOptionPanel = Vue.extend({
  ...
  methods: {
    ...
    changeMainMenuWidth (val: number) {
      this.$theme.settingTheme({
        mainMenuWidth: val,
      })
    },
    genCardTitle (h: CreateElement): VNode {
      const listItem = h(ZListItem, [
        h(ZListItemAvatar, { props: { color: this.$themeStore.primaryColor } }),
        h(ZListItemContent, [
          h(ZListItemTitle, { staticClass: 'headline' }, ['主题设置']),
        ]),
        h(ZListItemAction, [
          h(ZBtn, {
            props: { icon: true },
      ...
      const row = h(ZRow, { props: { noGutter: true } }, [
        h(ZCol, { props: { cols: 'auto' } }),
        h(ZSpacer),
        h(ZCol, { props: { cols: 'auto' } }),
      ])
      return h(ZCardTitle, {}, [
    ...
    genCardBody (h: CreateElement): VNode {
      const col1 = h(ZCol, { props: { col: 3 } }, [
        h(ZColorPicker, {
          staticClass: 'ma-2',
          props: {
      ...
      const col3 = h(ZCol, [
        ...
        h('div',
          { style: { transform: 'scale(0.8)', marginTop: '-10px', marginLeft: '25px', textAlign: 'left', color: '#999' } },
          ['开启时支持mini模式，关闭时支持隐藏显示']
        ),
        h(ZSwitch, {
          staticClass: 'mb-2 ml-2',
          props: {
      ...
      return h(ZCardText, [
        h(ZRow, { props: { noGutter: true, justify: 'start' } }, [
          col1,
          col2,
          col3,
          h(ZCol),
