...
import { VTabs, VTab, VTabsItems, VTabItem, VTabsSlider } from './index';
import generateZSizeable from '../../zui/util/generateZSizeable';
import "../../../src/components/VTabs/ZTabs.scss";
const Sizeable = generateZSizeable(['v-tabs-size--x-small', 'v-tabs-size--small', 'v-tabs-size--default', 'v-tabs-size--large', 'v-tabs-size--x-large']);
export const ZTabs = mixins(VTabs, Sizeable).extend({
  name: 'z-tabs',
  computed: {
    classes() {
      const sizeableClasses = this.sizeableClasses;
      return { ...VTabs.options.computed.classes.call(this),
        ...sizeableClasses
      };
    }
