...
export const ZMessage = mixins(ZSnackbar).extend({
  ...
  methods: {
    genWrapper() {
      const list = this.getMessageList();
      const wrapper = ZSnackbar.options.methods.genWrapper.call(this);
    ...
    resetLocation() {
      ...
      if (item) {
        ...
        item.el.style.top = `${this.offsetTop + (item.index - 1) * (this.itemGap + this.itemHeight)}px`; // 重置所有通知位置及其索引
