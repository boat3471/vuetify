import Vue from 'vue';
import { ZForm, ZBtn, ZCard, ZCardActions, ZCardSubtitle, ZCardTitle, ZContainer, ZRow, ZTextField, ZCheckbox, ZCardText } from '../../../components';
import { getSlot } from '../../../util/helpers';
import "../../../../src/zui/components/ZAdmin/styles/ZDefaultLogin.scss";
export const ZDefaultLogin = Vue.extend({
  ...
  data() {
    return {
      ...
      rules: {
        ...
        username: v => v && v.length >= 4 && v.length <= 20 || '用户名至少4个字符，最多20个字符',
        password: v => v && v.length >= 4 && v.length <= 20 || '密码至少6个字符，最多20个字符'
      },
      password: '',
      remember: false,
      loginPd: true,
  ...
  methods: {
      ...
      if (loginForm.validate()) {
        ...
        if (this.$auth.login) {
          this.$auth.login({
            ...
            if (this.$auth.verifyLogin && this.$auth.verifyLogin(res)) {// console.info('login success', res)
            } else {
              this.disabled = false;
            }
          }).catch(e => {
    ...
    genText(h) {
      ...
      ...this.genUsername(h), ...this.genPassword(h), this.genRemember(h)]);
